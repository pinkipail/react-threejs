...
/*         TweenMax.to(this.ss[0], 2, {alpha: 0}) */
   
    }

    estimationDirection = () => {       
        if(this.sliderState.numberCurrentSlide < this.sliderState.numberNextSlide)
            return true
        else 
            return false
    ...
    animatingNext = () =>  {
        ...
        const currentSlide = this.slides[this.sliderState.numberCurrentSlide]
        const nextSlide = this.slides[this.sliderState.numberNextSlide]
        ...
        const currentBullet = this.bullets[this.sliderState.numberCurrentSlide]
        const nextBullet = this.bullets[this.sliderState.numberNextSlide]
...
/*      .staggerFromTo(nextImages, 1.5, {yPercent: 150, scaleY: 1.5}, {yPercent: 0, scaleY: 1, ease: Expo.easeInOut}, 0, 1) */
    ...
    nextSlide = (numberNextSlide) => {
        if(numberNextSlide === 'up')
            numberNextSlide = this.sliderState.numberNextSlide - 1
        if(numberNextSlide === 'down')
            numberNextSlide = this.sliderState.numberNextSlide + 1
        if (this.sliderState.animating) return
        if (numberNextSlide === this.sliderState.numberCurrentSlide) return
        this.sliderState.animating = true

        this.sliderState.numberNextSlide = this.boundsСhecking(numberNextSlide)
        
        
        this.animatingNext()

        this.sliderState.numberCurrentSlide = this.boundsСhecking(numberNextSlide)
        
    }

    changeTexture() {
        this.mat.uniforms.texture1.value = this.textures[this.sliderState.numberCurrentSlide]
        this.mat.uniforms.texture2.value = this.textures[this.sliderState.numberNextSlide]
    }
    ...
    handlerScroll = () => {
        new WheelIndicator({
            callback: (e) => {
                if (e.direction === 'down')
                    this.nextSlide(this.sliderState.numberCurrentSlide + 1)
                else
                    this.nextSlide(this.sliderState.numberCurrentSlide - 1)
            }
        })
    ...
    render = ()=>{
        return (
            <div className="slider" ref={ref => (this.el = ref)}>
                ...
                {this.slideTexts.map((texts, idx)=> <Slide texts={texts} idx={idx} key={idx} gettingLink={this.gettingLink}/>)}
            
            <SliderNav handlerClick={this.nextSlide}/>
        </div>
        )
    }
}
